...
public class JSONUtils {
    ...
    public static ArrayList<Movie> getMoviesFromJSON(JSONObject jsonObject) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                Movie movie = new Movie(id, voteCount, title, originalTitle, overview, bigPosterPath, posterPath, backdropPath, voteAverage, releaseDate);
                result.add(movie);
            }
    ...
    public static ArrayList<Trailer> getTrailersFromJSON(JSONObject jsonObject) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject jsonObjectReviews = jsonArray.getJSONObject(i);
                String key = BASE_YOUTUBE_URL+jsonObjectReviews.getString(KEY_KEY_OF_VIDEO);
                String name = jsonObjectReviews.getString(KEY_NAME);
